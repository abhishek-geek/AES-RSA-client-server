...
const encrypt = (plaintext) => {
  ...
  let subNib = subNibbles(sBox, state);
  output.subNib1 = subNib;

  let sRow = shiftRows(subNib);
  output.shiftRow1 = sRow;

  state = mixColumns(sRow);
  output.mixCol = state;

  state = addRoundKey(round1Key, state);
  output.addRoundKey1 = state;

  subNib = subNibbles(sBox, state);
  output.subNib2 = subNib;

  state = shiftRows(subNib);
  output.shiftRow2 = state;
...
const decrypt = (ciphertext) => {
  ...
  state = subNibbles(InvSBox, shiftRows(state));
...
const AESEncrypt = (plainText, secretKey) => {
  ...
  return {cipherText: encrypt(plainText), output};
};
